...
from gezet.helper.utility import get_arg
...
async def pytrans_tr(_, message: Message):
  ...
  if r_msg:
    if r_msg.text:
      ...
      return await tr_msg.edit("`Balas pesan yang berisi teks!`")
    # Checks if dest lang is defined by the user
    if not args:
      return await tr_msg.edit(f"`Tentukan bahasa tujuan!` \n\n**Ex:** `{Config.CMD_PREFIX}ptr si Hei, saya menggunakan telegram!`")
...
"""
    if len(tred_txt) > 4096:
      await tr_msg.edit("`Wah!! Teks yang Diterjemahkan Begitu Lama!, Tunggu sebentar, saya akan mengirimkannya sebagai file!`")
...
add_command_help(
    "translate",
    [
        [".tr", "Terjemahkan beberapa teks dengan memberikan teks atau membalas teks/keterangan itu."],
    ],
)
