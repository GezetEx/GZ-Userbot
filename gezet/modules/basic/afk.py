...
from gezet.helper.PyroHelpers import GetChatID, ReplyCheck
...
async def afk_unset(bot: Client, message: Message):
    ...
    if AFK:
        ...
        await message.edit(
            f"`Saat kau pergi (for {last_seen}), Anda telah menerima {sum(USERS.values()) + sum(GROUPS.values())} "
            f"pesan dari {len(USERS) + len(GROUPS)} obrolan`"
        )
        AFK = False
        AFK_TIME = ""
        AFK_REASON = ""
