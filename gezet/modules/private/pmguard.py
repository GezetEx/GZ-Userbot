...
from gezet.database.pmpermitdb import get_approved_users, pm_guard
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await gezet.set_block_message(gezet.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await gezet.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await gezet.get_pm_settings()
    await gezet.allow_user(chat_id)
